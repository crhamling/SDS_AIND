...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        #Acquire the previous state levels
        prev_S_level = self.s_levels[level]
        #Initialize action level
        self.a_levels.append(set()) 
        
        for action in self.all_actions:
            #Make action node for this action
            action_node = PgNode_a(action)
            #Checks if a proposed PgNode_a has prenodes that are a subset of the previous S level.
            if action_node.prenodes.issubset(prev_S_level):
                for state_node in prev_S_level:
                    #Connect the nodes
                    action_node.parents.add(state_node)
                    state_node.children.add(action_node)
                #adds action nodes to the current level in self.a_levels[level]
    ...
    def add_literal_level(self, level):
        ...
        #Acquire the previous action levels
        prev_A_level = self.a_levels[level - 1] 
        #Initialize state level
        self.s_levels.append(set()) 
        
        #Identify all effect nodes of the previous action nodes and add to new S level
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        #Boolean values of whether node_a2 negates node_a1, and vice versa
        node_a1_negated = set(node_a1.action.effect_add) & set(node_a2.action.effect_rem)
        node_a2_negated = set(node_a2.action.effect_add) & set(node_a1.action.effect_rem)
        #Returns true if one actions negate the effect of the other
        if (node_a1_negated | node_a2_negated):
            return True
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        """
        #Boolean values of whether node_a2 negates node_a1, and vice versa
        node_a1_negated = (set(node_a1.action.effect_add) & set(node_a2.action.precond_neg) | 
                           set(node_a1.action.effect_rem) & set(node_a2.action.precond_pos) )
        node_a2_negated = (set(node_a2.action.effect_add) & set(node_a1.action.precond_neg) |
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        #Competing needs can be identified by checking parent mutual exclusion
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        :return: bool
        """
        #If two nodes have the same symbol but different literals, then it's a negation
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for s1_parent in node_s1.parents:
            for s2_parent in node_s2.parents:
                #All parents of the two nodes need to be mutually eclusive in order to achieve inconsistent support
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal:
            #Create a node for the goal state
