...
def minimax_decision(gameState):
    ...
    # This Solution using an explicit loop based on max_value()
    best_score = float("-inf")                                      # Start with negative Infinity as best score
    best_move = None                                                # No best move yet
    for m in gameState.get_legal_moves():                           # For all of the legal moves (m) in the game state (of the virgin board)
        v = min_value(gameState.forecast_move(m))                   # Set V to min_value of each possible move in the gamestate
        if v > best_score:                                          # If V is > best score, set best score to V and Best Move to M
            best_score = v
            best_move = m
    return best_move
...
# Print out all of the GameStates
...
#
# Test from Putting it all Together
#
