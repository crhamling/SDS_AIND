...
postscript -->
    "/init { /N exch def 340 N div dup scale -1 -1 translate \c
             /Palatino-Roman 0.8 selectfont 0 setlinewidth \c
             1 1 N { 1 1 N { 1 index c } for pop } for } bind def \c
     /r { translate 0 0 1 1 4 copy rectfill 0 setgray rectstroke } bind def \c
     /i { gsave 0.5 setgray r grestore } bind def \c
     /q { gsave r 0.5 0.28 translate (Q) dup stringwidth pop -2 div 0 moveto \c
          1 setgray show grestore } bind def \c
...
show(N, Options, Qs) :-
        N #> 0,
        n_queens(N, Qs),
        open(pipe('gswin64 -dNOPROMPT -g680x680 -dGraphicsAlphaBits=2 -r144 -q'),
