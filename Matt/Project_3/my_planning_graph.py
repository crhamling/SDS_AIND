...
class PlanningGraph():
    ...
    def add_literal_level(self, level):
        ...
        # Initialize this level of states
        ...
        for a_node in previous_a_level:
            ...
            for effect_node in a_node.effnodes:
                ...
                # Connect state level to action and the reverse
                a_node.children.add(effect_node)
                effect_node.parents.add(a_node)
                
                # Add a new state to this new level of states
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        #
        # Readings on "Competing Needs" of MUTEX Relationships
        ...
        #
        return bool(
            # Test that actions from node 1 negated by actions in node 2?
            set(node_a1.action.effect_add) & set(node_a2.action.effect_rem) |

            # Test that actions from node 2 negated by actions in node 1?
            set(node_a2.action.effect_add) & set(node_a1.action.effect_rem)
        )        


    #
    # FN - interference_mutex
    #
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        #
        return bool(
            # Test that actions in node 1 the negation of a precondition of node 2?
            set(node_a1.action.effect_add) & set(node_a2.action.precond_neg) |
            set(node_a1.action.effect_rem) & set(node_a2.action.precond_pos) |

            # Test that actions in node 2 the negation of a precondition of node 1?
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Definition: one of the preconditions of one action is mutually exclusive with a precondition of another
        #
        # Readings on "Competing Needs" of MUTEX Relationships
        # http://pages.mtu.edu/~nilufer/classes/cs5811/2009-fall/lecture-slides/cs5811-ch11b-graphplan.pdf
        # https://www.cs.umd.edu/~nau/planning/slides/chapter06.pdf
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        # Verify both states represent the same symbol
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        # Make sure node_s1's preconditions aren't mutually exclusive (they cannot both be true) with node_s2's preconditions 
        # NOTE: The PgNode.is_mutex method tests whether two nodes are mutually exclusive - very handy.
        # The concept of Inconsistent Support of Mutual Exclusion is that ALL ways of achieving tow literals is Mutually Exclusive.
        #
        # Readings on "Inconsistent Support" of MUTEX Relationships
        # http://pages.mtu.edu/~nilufer/classes/cs5811/2009-fall/lecture-slides/cs5811-ch11b-graphplan.pdf
        # https://www.cs.umd.edu/~nau/planning/slides/chapter06.pdf
        #
    ...
    def h_levelsum(self) -> int:
        ...
        _goals = self.problem.goal
        s_levels = self.s_levels
        
        # For each goal in the problem, determine their level cost, add them togethe, return the sum
        #
        for goal in _goals:
