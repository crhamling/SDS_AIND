...
class AirCargoProblem(Problem):
            ...
            for cargo in self.cargos:
                for plane in self.planes:
                    for airport in self.airports:
                        # There is a cargo and a plane at the airport
                        precond_pos = [
                        ...
                        # Remove cargo from the airport
                        effect_rem = [expr("At({}, {})".format(cargo, airport))]
                        # Create a Action object to be loaded
                        load = Action(
            ...
            for cargo in self.cargos:
                for plane in self.planes:
                    for airport in self.airports:
                        ...
                        precond_neg = []
                        # Add the cargo to the airport
                        effect_add = [expr("At({}, {})".format(cargo, airport))]
                        # Remove cargo from the plane
                        effect_rem = [expr("In({}, {})".format(cargo, plane))]
                        # Create a Action object to be unloaded
                        unload = Action(
    ...
    def actions(self, state: str) -> list:
        ...
        kb = PropKB()
        # Add the current state to propositional logic knowledge base
        kb.tell(decode_state(state, self.state_map).pos_sentence())
        possible_actions = []
        for action in self.actions_list:

            # For every action, by default, assume action is possible
            is_action_possible = True

            # Loop negative clauses
            for clause in action.precond_neg:
                if clause in kb.clauses:
                    is_action_possible = False
                    break # Action is not possible, halt search

            # Only check if action is still possible
            if is_action_possible:
                # Loop positive clauses
                for clause in action.precond_pos:
                    if clause not in kb.clauses:
                        is_action_possible = False
                        break # Action is not possible, halt search

            # If the action is possible, add it to the list of possible actions
            if is_action_possible: 
    ...
    def result(self, state: str, action: Action):
        ...
        # Insert Add into next state positive, if not already added
        for fluent in action.effect_add:
            if fluent not in next_state.pos:
                next_state.pos.append(fluent)
        # Insert Remove into next state negative, if not already added 
